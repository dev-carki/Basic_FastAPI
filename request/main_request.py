...
async def read_item(request: Request):
    client_host = request.client.host # client IP 정보
    headers = request.headers # 딕셔너리 형태의 헤더 정보
    query_params = request.query_params # 쿼리 파라미터
    url = request.url # url
    path_params = request.path_params # 패스 파라미터
    http_method = request.method # HTTP Method
